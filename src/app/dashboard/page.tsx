          ...
          <nav className="mt-8">
            <div className={`${sidebarCollapsed ? 'px-1' : 'px-6'} space-y-4`}>
              <button
                ...
                className={`flex items-center ${sidebarCollapsed ? 'justify-center' : 'justify-start space-x-3'} w-full px-4 py-3 rounded-lg transition-all duration-200 ${currentPage === 'dashboard'
              ...
              <button
                ...
                className={`flex items-center ${sidebarCollapsed ? 'justify-center' : 'justify-start space-x-3'} w-full px-4 py-3 rounded-lg transition-all duration-200 ${currentPage === 'groups'
                  ? 'bg-blue-50 text-blue-700 border-r-2 border-blue-600'
                  : 'text-gray-600 hover:bg-gray-50'
                  }`}
              ...
              <button
                ...
                className={`flex items-center ${sidebarCollapsed ? 'justify-center' : 'justify-start space-x-3'} w-full px-4 py-3 rounded-lg transition-all duration-200 ${currentPage === 'settle'
                  ? 'bg-blue-50 text-blue-700 border-r-2 border-blue-600'
                  : 'text-gray-600 hover:bg-gray-50'
                  }`}
              ...
              <button
                ...
                className={`flex items-center ${sidebarCollapsed ? 'justify-center' : 'justify-start space-x-3'} w-full px-4 py-3 rounded-lg transition-all duration-200 ${currentPage === 'profile'
                  ? 'bg-blue-50 text-blue-700 border-r-2 border-blue-600'
                  : 'text-gray-600 hover:bg-gray-50'
                  }`}
                    ...
                    {notifications.length === 0 ? (
                      ...
                      <div className="py-2">
                        {notifications.map((notification) => (
                          <div
                            key={notification.id}
                            className={`px-4 py-3 hover:bg-gray-50 transition-colors duration-150 cursor-pointer border-l-4 ${notification.isRead
                    ...
                    {dropdownItems.map((item, index) => (
                      <React.Fragment key={index}>
                        <button
                          ...
                          className={`w-full px-4 py-2 text-left flex items-center space-x-3 hover:bg-gray-50 transition-colors duration-150 ${item.danger ? 'text-red-600 hover:bg-red-50' : 'text-gray-700'
                            }`}
                        >
...
// }, []);
 ...
 console.log(currentUser);
      ...
      <div className="fixed inset-0 transition-opacity bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                {groupIcons.map((icon) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-xl transition-all duration-200 bg-gradient-to-r ${isSelected
          ...
          <div className="flex gap-3 pt-2 m-3">
            ...
            <button
              type="submit"
              disabled={!formData.name.trim() || loading || !currentUser}
  ...
  const Dashboard = () => {
    ...
    // Displaying the groups on the dashboard
...
activityMap.set(doc.id, {
  ...
  return Array.from(activityMap.values());
    ...
    const fetchUserExpenses = async (uid: string): Promise<Activity[]> => {
      ...
      const [createdSnap, sharedSnap] = await Promise.all([
        getDocs(createdQuery),
        getDocs(sharedQuery)
      ]);

      const allDocs = [...createdSnap.docs, ...sharedSnap.docs];
      const activityMap = new Map<string, Activity>();

      allDocs.forEach(doc => {
        const data = doc.data();
        const timeAgo = formatTimeAgo(data.createdAt?.toDate?.());
        const userName = data.createdBy?.name || 'Someone';

        activityMap.set(doc.id, {
          user: userName,
          action: `added ₦${data.amount} for ${data.title}`,
          group: data.groupName || 'Unnamed Group',
          time: timeAgo,
 ...
 const [loading, setLoading] = useState(true);
          ...
          <div className="p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                ...
                <p className="text-2xl font-bold text-gray-800">{stats.groupCount}</p>
              </div>
              ...
              <div className="divide-y divide-gray-100">
                {activities.length === 0 ? (
                  <div className="p-10 text-center text-gray-500">
                    ...
                    <p className="text-sm mt-2">Once you start creating groups or adding expenses, they’ll show up here.</p>
                  </div>
                ) : (
                  <div className="max-h-[400px] overflow-y-auto divide-y divide-gray-10">
  ...
  const AddExpenseModal = () => {
    ...
    const handleSubmit = async () => {
      ...
      const cleanSelectedMembers = selectedMembers.filter(uid => typeof uid === 'string' && uid);

      const expenseData = {
        ...formData,
        splitType,
        sharedWith: cleanSelectedMembers,
        groupName: selectedGroup.name, // ✅ Add group name here
              ...
              <div>
                ...
                <div className="flex space-x-3">
                  <button
                    ...
                    className={`flex-1 px-4 py-3 rounded-lg border-2 transition-all duration-200 ${splitType === 'equal'
                  ...
                  <button
                    ...
                    className={`flex-1 px-4 py-3 rounded-lg border-2 transition-all duration-200 ${splitType === 'custom'
                      ? 'border-purple-600 bg-purple-50 text-purple-700'
                      : 'border-gray-300 hover:border-gray-400'
                      }`}
                  >
                ...
                <div className="grid grid-cols-2 gap-2">
                  {categories.map(category => (
                    <button
                      ...
                      className={`p-3 rounded-lg border-2 transition-all duration-200 text-purple-500 ${formData.category === category.id
